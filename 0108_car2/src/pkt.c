...
void *sub_thread_pkt_rx(void *arg) {
    ...
    while (g_keep_running) {
        ...
        if (rx_pkt) {
            ...
            DBG_INFO("PKT-RX: Incoming Packet %lX)", rx_pkt->accident);
            DBG_INFO("PKT-RX: Incoming Packet (Sender: 0x%lX, Accident: 0x%lX, Heading: 0x%X)", 
                     rx_pkt->sender.sender_id, rx_pkt->accident.accident_id, rx_pkt->accident.direction);
            // 2. VAL 스레드(T4)로 데이터 
            ...
            //  판단 모듈(VAL, T4)로 전달하여 1km 필터링 및 사고 관리 수행
